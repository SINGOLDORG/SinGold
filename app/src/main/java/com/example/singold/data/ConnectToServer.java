...
public  class ConnectToServer {
    ...
    private static MobileServiceTable<MatchingSurvey> MatchingSurveyTable;
...
//                            }
    ...
    public static void addInTable(final MatchingSurvey item) throws ExecutionException, InterruptedException {
        ;
        if (MatchingSurveyTable == null)
            MatchingSurveyTable = mClient.getTable(MatchingSurvey.class);
        AsyncTask<Void, Void, Void> task = new AsyncTask<Void, Void, Void>() {
            @Override
            protected Void doInBackground(Void... params) {
                try {
                    final MatchingSurvey entity = MatchingSurveyTable.insert(item).get();
